...
export function ShareCard({ share, isEditMode = false, onUpdate, onDelete }: ShareCardProps) {
	...
	return (
		<motion.div
			...
			{isEditMode && (
				<div className='absolute top-3 right-3 z-10 flex gap-2 items-center bg-white/80 px-2 py-1 rounded-lg backdrop-blur-sm'>
					{isEditing ? (
						<>
							<button onClick={(e) => { e.stopPropagation(); handleCancel() }} className='rounded px-1.5 py-1 text-xs text-gray-500 transition-colors hover:text-gray-700'>
								取消
							</button>
							<button onClick={(e) => { e.stopPropagation(); setIsEditing(false) }} className='rounded px-1.5 py-1 text-xs text-blue-500 transition-colors hover:text-blue-700'>
								完成
							</button>
						</>
					) : (
						<>
							<label className='flex items-center gap-1 cursor-pointer text-[10px] text-gray-600 font-medium mr-1'>
								<input 
									type='checkbox' 
							...
							<button 
								onClick={(e) => { e.stopPropagation(); handleFieldChange('isPinned', !localShare.isPinned) }} 
								className={cn('p-1 rounded transition-colors', localShare.isPinned ? 'text-brand bg-brand/10' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600')}
							...
							<button onClick={(e) => { e.stopPropagation(); setIsEditing(true) }} className='rounded px-1.5 py-1 text-xs text-blue-500 transition-colors hover:text-blue-700'>
								编辑
							</button>
							<button onClick={(e) => { e.stopPropagation(); onDelete?.() }} className='rounded px-1.5 py-1 text-xs text-red-500 transition-colors hover:text-red-700'>
								删除
							</button>
						</>
					)}
			...
			<div className={cn('p-6', !canEdit && 'cursor-pointer')} onClick={() => !canEdit && setIsDetailOpen(true)}>
				...
				<p
					...
					className={cn(
						...
						canEdit ? 'cursor-text' : 'line-clamp-3'
					)}>
			...
			{isDetailOpen && <ShareDetailModal share={localShare} onClose={() => setIsDetailOpen(false)} />}
		</motion.div>
	)
}
