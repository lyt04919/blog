					...
					{isEditing ? (
						...
						<>
							<label className='flex items-center gap-1 cursor-pointer text-[10px] text-gray-600 font-medium mr-1'>
								<input 
									type='checkbox' 
									checked={localBlogger.isShow || false} 
							...
							<button 
								onClick={(e) => { e.stopPropagation(); handleFieldChange('isPinned', !localBlogger.isPinned) }} 
								className={cn('p-1 rounded transition-colors', localBlogger.isPinned ? 'text-brand bg-brand/10' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600')}
								title={localBlogger.isPinned ? '取消置顶' : '置顶'}
							>
								<Pin className={cn('w-3.5 h-3.5', localBlogger.isPinned && 'fill-current')} />
							</button>
