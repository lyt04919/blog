...
export function FavoriteItemCreateDialog({ targetType, onClose, onSave }: FavoriteItemCreateDialogProps) {
	const [localItem, setLocalItem] = useState<FavoriteItem>({
		...
		isPinned: false,
		isShow: true,
		stars: targetType === 'games' ? 5 : undefined,
		status: targetType === 'games' ? '正在玩' : undefined,
		embedCode: targetType === 'music' ? '' : undefined,
		category: ''
	})
	...
	return (
		<DialogModal open onClose={onClose} className='card max-w-3xl w-full max-h-[90vh] p-8 md:p-10 relative bg-white flex flex-col shadow-2xl overflow-y-auto'>
			...
			<button 
				onClick={onClose} 
				className='absolute top-8 right-8 md:top-10 md:right-10 z-20 p-2 text-neutral-400 hover:text-neutral-900 bg-neutral-50 hover:bg-neutral-100 rounded-full transition-colors'
			>
				<X className='w-5 h-5' />
			</button>
